...
idx_to_drop  = data[data['price'] > 1400000].index
idx_to_drop2 = data[data['price'] < 500].index
data.drop(idx_to_drop, inplace=True)
data.drop(idx_to_drop2, inplace=True)
...
#Drop rows based on heatmap
data = data.dropna(how='all', subset=['condition', 'cylinders'])
data.reset_index(drop=True, inplace=True)
msno.heatmap(data)

#Drop rows based on heatmap
data = data.dropna(how='all', subset=['drive', 'cylinders'])
data.reset_index(drop=True, inplace=True)
msno.heatmap(data)
# msno.bar(data)
...
most_ts = data['title_status'].value_counts().index[0] #gas 93%
...
#Start to deal with categorical data
unique_val = {}
...
model.save('car_prices_model.h5') 
